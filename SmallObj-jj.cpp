    ...
    , allocChunk_(0)
{
    ...
    // jjhou: 以下很有趣的是，為什麼使用 DEFAULT_CHUNK_SIZE 而不是
    //        SmallObjAllocator::chunkSize_。此外，後者完全沒有被用到。
...
FixedAllocator::~FixedAllocator()
{
    ...
    for (; i != chunks_.end(); ++i)
    {
       // std::cout << (int)i->blocksAvailable_ << ' ' << (int)numBlocks_ << std::endl;
